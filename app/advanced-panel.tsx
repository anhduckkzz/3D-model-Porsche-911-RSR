...
const stages=[
 ...
 {title:'Gia cố trụ bằng beam ghép so le',body:'Mỗi bên giữ tam giác đáy 48 × 64 × 80 mm. Thêm beam 15L phía sau trụ, lệch lên một khoảng lỗ; phần trên thêm beam 7L ở lớp kế tiếp. Ba pin 3L khóa các vùng nối ba lớp, các pin 2L nối dọc thân. Mối nối của hai lớp không trùng nhau. Luồn axle 8L tại đỉnh giằng dưới, thêm bush phía sau và spacer; để đầu trước chờ giằng bước tiếp theo.'},
 {title:'Khóa cầu và giằng đầu trụ',body:'Lắp hai cầu ngang cách cao độ 16 mm, khóa vào cả hai lớp trụ bằng pin 3L. Mỗi bên thêm một giằng 11L từ cầu xuống đỉnh tam giác dưới: pin 2L ở đầu cầu, axle 8L xuyên cụm ở đầu dưới. Đẩy bush phía trước sát giằng để giữ chồng mảnh giữa hai bush. Tam giác mới đưa tải từ cầu vào giữa trụ; mui không mang tải.'},
...
type Props={data:ModelData|null;step:number;onStep:(n:number)=>void;context:boolean;onContext:(v:boolean)=>void};
export default function AdvancedPanel({data,step,onStep,context,onContext}:Props){
 ...
 return <>
 <div className="advanced-title"><span className="small-label">CAMERA RIG / 05</span><h1>Add-on camera cage.</h1><p>Giữ nguyên xe · Khóa bằng LEGO · Vsmart Aris</p></div>
 <div className="advanced-view-options"><button aria-pressed={!context} onClick={()=>onContext(false)}>Cụm lắp ráp</button><button aria-pressed={context} onClick={()=>onContext(true)}>Trên xe nguyên vẹn</button></div>
 <aside className="advanced-panel" aria-label="Thiết kế giá đỡ điện thoại">
  ...
  <details><summary>Kích thước và danh sách mảnh</summary><dl><div><dt>Aris theo asset</dt><dd>156,55 × 76,18 × 10,71 mm</dd></div><div><dt>Khoảng neo mỗi bên</dt><dd>32 mm · 4 khoảng lỗ</dd></div><div><dt>Giằng mỗi bên</dt><dd>2 tam giác · 48 × 64 × 80 mm</dd></div><div><dt>Tâm hai trụ ngoài</dt><dd>304 mm</dd></div></dl>{review&&mountInventory(review.plan.parts).map(p=><p key={p.code}>{p.quantity} × {p.code} — {p.name}</p>)}<p>Mảnh dùng đúng geometry và kích thước LDraw có sẵn. Axle được trượt để chỉnh chặn, không kéo giãn asset.</p><a href="/camera-bridge-design.md" target="_blank" rel="noreferrer">Bản thiết kế và trình tự lắp chi tiết ↗</a></details>
