...
export default function DriveConsole({onState}:{onState:(state:VehicleState)=>void}){
 ...
 useEffect(()=>{
  let mounted=true;const current=new VehicleLink(s=>{if(mounted){setState(s);report.current(s);if(!s.armed)intent.current.clear()}});link.current=current;
  ...
  return()=>{mounted=false;report.current({connected:false,armed:false,fb:0,lr:0,message:'Chưa kết nối'});clearInterval(tick);if(pulseTimer.current)clearTimeout(pulseTimer.current);intent.current.clear();void current.close();window.removeEventListener('keydown',down);window.removeEventListener('keyup',up);window.removeEventListener('blur',halt);window.removeEventListener('pagehide',halt);document.removeEventListener('visibilitychange',hidden)};
 },[]);
 ...
 return <>
  ...
  <span className="drive-motion-label">{state.connected?'Chuyển động minh họa theo lệnh · không phải vị trí đo':'Kết nối xe để điều khiển'}</span>
