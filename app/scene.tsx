...
const Scene=forwardRef<SceneHandle,Props>(function Scene(props,ref){
 ...
 useEffect(()=>{
  ...
  function clearPreview(){for(const child of previewGroup.children)if(child instanceof T.InstancedMesh)child.dispose();previewGroup.clear()}
  function rebuildPreview(){
   ...
   if(!previewRenderer){try{previewRenderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'low-power'});previewRenderer.setPixelRatio(Math.min(devicePixelRatio,1.5));previewRenderer.setClearColor(0xf6f7f8);previewRenderer.outputColorSpace=T.SRGBColorSpace;previewRenderer.toneMapping=T.ACESFilmicToneMapping;previewRenderer.toneMappingExposure=1.0;host.appendChild(previewRenderer.domElement);previewRenderer.domElement.setAttribute('aria-label','Các mảnh cần dùng trong bước này, xem trong không gian 3D');previewControls=new OrbitControls(previewCamera,previewRenderer.domElement);previewControls.enablePan=false;previewControls.enableDamping=false;previewControls.minZoom=.6;previewControls.maxZoom=5;previewControls.addEventListener('change',()=>{previewDirty=true;request()});previewScene.add(new T.HemisphereLight(0xffffff,0x9ca5ad,1.4));const light=new T.DirectionalLight(0xffffff,1.8);light.position.set(4,8,5);previewScene.add(light);previewResize=new ResizeObserver(()=>{previewDirty=true;previewKey='';rebuildPreview();request()});previewResize.observe(host)}catch{return}}
   ...
   if(key===previewKey)return;previewKey=key;clearPreview();
   ...
   if(attachment){
    ...
    const grouped=new Map<number,number[]>();for(const id of ids){const geo=model.parts[id].geo;if(!grouped.has(geo))grouped.set(geo,[]);grouped.get(geo)!.push(id)}
    for(const [geo,partIds] of grouped){const mesh=new T.InstancedMesh(geometries[geo],previewMaterial,partIds.length);partIds.forEach((id,index)=>{const ps=base[id];matrix.compose(ps.position.clone().sub(center).multiplyScalar(factor),ps.quaternion,ps.scale.clone().multiplyScalar(factor));mesh.setMatrixAt(index,matrix)});mesh.instanceMatrix.needsUpdate=true;mesh.computeBoundingSphere();previewGroup.add(mesh)}
   }else{
  ...
  const previewMaterial=new T.MeshStandardMaterial({vertexColors:true,roughness:.4,metalness:.01,side:T.DoubleSide});mats.push(previewMaterial);
  ...
  async function start(){try{
   renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});renderer.setClearColor(0xf1f3f4);renderer.setPixelRatio(pixelRatio);renderer.setSize(el.clientWidth,el.clientHeight,false);renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=.92;el.appendChild(renderer.domElement);renderer.domElement.setAttribute('aria-label',props.car.name+' tương tác. Kéo để xoay, cuộn để zoom, chạm để chọn mảnh.');renderer.domElement.tabIndex=0;
   ...
   const generator=new T.PMREMGenerator(renderer),room=new RoomEnvironment();environment=generator.fromScene(room,.04);scene.environment=environment.texture;room.dispose();generator.dispose();scene.add(new T.HemisphereLight(0xffffff,0x969da6,.9));const key=new T.DirectionalLight(0xffffff,2);key.position.set(-4,10,6);scene.add(key);const fill=new T.DirectionalLight(0xffffff,.45);fill.position.set(7,4,-7);scene.add(fill);
  ...
  start();return()=>{disposed=true;worker?.terminate();if(raf)cancelAnimationFrame(raf);resize?.disconnect();previewResize?.disconnect();controls?.dispose();previewControls?.dispose();cleanups.forEach(fn=>fn());clearArrows();clearPreview();marker.dispose();batches.forEach(b=>b.mesh.dispose());geometries.forEach(g=>g.dispose());mats.forEach(m=>m.dispose());environment?.dispose();renderer?.dispose();renderer?.forceContextLoss();previewRenderer?.dispose();previewRenderer?.forceContextLoss();renderer?.domElement.remove();previewRenderer?.domElement.remove();mount?.dispose();driveMount?.dispose();road.geometry.dispose();(road.material as T.Material).dispose();wake.current=null;apply.current=null;api.current=null};
 },[]);
