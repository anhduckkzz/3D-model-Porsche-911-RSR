...
import {createFlatLayout,groupOffsets,insertionOffset,type PiecePose,FLAT_ROTATION} from './explosion-layout';
...
type Props={step:number;explode:number;group:string;selected:number|null;mode:ViewerMode;replay:number;followStep:boolean;onReady:(d:ModelData)=>void;onSelect:(id:number|null)=>void;previewHost:RefObject<HTMLDivElement|null>};
...
const Scene=forwardRef<SceneHandle,Props>(function Scene(props,ref){
 ...
 useEffect(()=>{apply.current?.()},[props.step,props.explode,props.group,props.selected,props.mode,props.replay,props.followStep]);
 useEffect(()=>{
  ...
  let motions:Motion[]=[],motionStart=0,motionDuration=900,lastKey='',lastSelection:number|null=null,lastMode:ViewerMode='explore',lastStep=-1,lastReplay=0,layoutKey='',flat=false,span=12,aspect=1,lowDetail=window.innerWidth<768,pixelRatio=Math.min(devicePixelRatio,1.7),dirty=true;
  ...
  function write(p:Part,ps:PiecePose){const slot=slots.get(p.id);if(!slot)return;matrix.compose(ps.position,ps.quaternion,ps.scale);slot.batch.mesh.setMatrixAt(slot.index,matrix);slot.batch.mesh.instanceMatrix.needsUpdate=true;current[p.id]=clonePose(ps)}
  ...
  function viewport(){const narrow=el.clientWidth<700,building=latest.current.mode==='build';const left=narrow?24:36,right=building&&!narrow?320:36,top=narrow?190:105,bottom=narrow?(building?350:165):175;return {left,right,top,bottom,width:Math.max(120,el.clientWidth-left-right),height:Math.max(120,el.clientHeight-top-bottom)}}
  ...
  function autoFit(instant=false){
   ...
   if(s.mode==='build'&&s.followStep){const fresh=visible.filter(p=>p.step===s.step);if(fresh.length)focus=fresh}
  ...
  function stylePieces(){
   const s=latest.current;for(const b of batches){for(let j=0;j<b.ids.length;j++){const p=model.parts[b.ids[j]];b.styles.setX(j,p.id===s.selected?2:s.mode==='build'&&p.step===s.step?1:0)}b.styles.needsUpdate=true}
  ...
  function rebuildPreview(){
   const s=latest.current;const ids=s.mode==='build'?(model.steps[s.step-1]?.parts??[]):s.selected!==null?[s.selected]:[];const unique=[...new Set(ids.map(id=>model.parts[id].geo))];
   const key=s.mode+':'+unique.join(',');const host=s.previewHost.current;if(!host)return;
   ...
   const w=Math.max(1,host.clientWidth),h=Math.max(1,host.clientHeight),a=w/h;const half=Math.max(rows*1.65,columns*1.65/a,2)/2*1.12;previewCamera.left=-half*a;previewCamera.right=half*a;previewCamera.top=half;previewCamera.bottom=-half;previewCamera.zoom=1;previewCamera.position.set(0,0,20);previewCamera.lookAt(0,0,0);previewCamera.updateProjectionMatrix();previewControls?.target.set(0,0,0);previewControls?.update();previewRenderer.setSize(w,h,false);previewDirty=true;
  }
  ...
  function updateState(){
   ...
   const key=[s.mode,s.step,s.group,extent.toFixed(4),s.replay,s.followStep,aspect.toFixed(3)].join(':');
   if(key===lastKey){stylePieces();rebuildPreview();request();return}
   const enteringBuild=s.mode==='build'&&lastMode!=='build';const animateBuild=s.mode==='build'&&(s.step!==lastStep||s.replay!==lastReplay||enteringBuild);
   lastKey=key;lastMode=s.mode;lastStep=s.step;lastReplay=s.replay;
   const previousVisible=new Set(slots.keys());visible=model.parts.filter(p=>(s.group==='all'||p.group===s.group)&&(s.mode==='explore'||p.step<=s.step));
   const nextLayoutKey=[s.mode,s.mode==='build'?s.step:0,s.group,availableAspect().toFixed(3)].join(':');if(!layout||layoutKey!==nextLayoutKey){layout=createFlatLayout(visible,model,availableAspect());layoutKey=nextLayoutKey}const offsets=groupOffsets(model);destinations=base.map(clonePose);
   for(const p of visible){const target=destinations[p.id];if(extent<=.4){target.position.addScaledVector(offsets.get(p.group)!,smooth(extent/.4))}else{const t=smooth((extent-.4)/.6);target.position.add(offsets.get(p.group)!);const goal=layout.poses.get(p.id)!;target.position.lerp(goal.position,t);target.quaternion.slerp(goal.quaternion,t);target.scale.lerp(goal.scale,t)}}
   ...
   for(const p of visible){const to=destinations[p.id],from=current[p.id]?clonePose(current[p.id]):clonePose(to);const fresh=s.mode==='build'&&p.step===s.step&&animateBuild;
    if(fresh){const offset=insertionOffset(p,model);from.position.copy(to.position).add(offset);from.quaternion.copy(to.quaternion);if(newIndex<18){const localCenter=new T.Vector3().fromArray(model.geometries[p.geo].bounds[0]).add(new T.Vector3().fromArray(model.geometries[p.geo].bounds[1])).multiplyScalar(.5);const end=localCenter.multiply(to.scale).applyQuaternion(to.quaternion).add(to.position);const arrow=new T.ArrowHelper(offset.clone().normalize().negate(),end.clone().add(offset),offset.length()*.8,0xb95426,.16,.075);arrows.add(arrow)}newIndex++}
    else if(!previousVisible.has(p.id))Object.assign(from,clonePose(to));
    write(p,from);
    if(from.position.distanceToSquared(to.position)>1e-8||Math.abs(from.quaternion.dot(to.quaternion))<.999999)motions.push({part:p,from,to,delay:fresh?Math.min(.18,newIndex*.008):0});else write(p,to);
   }
   ...
   flat=extent>.92;if(controls){controls.enableRotate=!flat;controls.mouseButtons.LEFT=flat?T.MOUSE.PAN:T.MOUSE.ROTATE;controls.touches.ONE=flat?T.TOUCH.PAN:T.TOUCH.ROTATE}
   if(s.mode==='explore'||s.followStep||enteringBuild)autoFit(!current.length);rebuildPreview();request();
  }
  function frame(now:number){
   ...
   if(motions.length||cameraMotion||moving||dirty)request();else lastFrame=0;
  }
  async function start(){try{
   ...
   const up=(e:PointerEvent)=>{pointers.delete(e.pointerId);if(multi||dragged||Math.hypot(e.clientX-sx,e.clientY-sy)>6||!renderer)return;const rect=renderer.domElement.getBoundingClientRect();pointer.set((e.clientX-rect.left)/rect.width*2-1,-(e.clientY-rect.top)/rect.height*2+1);ray.setFromCamera(pointer,camera);const hits=ray.intersectObjects(batches.map(b=>b.mesh),false);if(hits[0]){const b=batches.find(b=>b.mesh===hits[0].object)!;latest.current.onSelect(b.ids[hits[0].instanceId!]);return}if(flat&&layout){const point=ray.ray.intersectPlane(new T.Plane(new T.Vector3(0,0,1),0),new T.Vector3());if(point){for(const cell of layout.cells.values()){if(Math.abs(point.x-cell.x)<=cell.width/2&&Math.abs(point.y-cell.y)<=cell.height/2){latest.current.onSelect(cell.id);return}}}}latest.current.onSelect(null)};
  ...
  start();return()=>{disposed=true;worker?.terminate();if(raf)cancelAnimationFrame(raf);resize?.disconnect();previewResize?.disconnect();controls?.dispose();previewControls?.dispose();cleanups.forEach(fn=>fn());clearArrows();marker.dispose();batches.forEach(b=>b.mesh.dispose());geometries.forEach(g=>g.dispose());mats.forEach(m=>m.dispose());environment?.dispose();renderer?.dispose();previewRenderer?.dispose();renderer?.domElement.remove();previewRenderer?.domElement.remove();apply.current=null;api.current=null};
 },[]);
