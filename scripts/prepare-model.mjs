...
const output=path.join('public',entry.assetPath);
...
const dirnames={chassis:'Khung & kết nối',engine:'Động cơ & truyền động',suspension:'Hệ treo & tay lái',cockpit:'Khoang lái',doors:'Cửa xe',roof:'Mui & thân trên',rear:'Đuôi & cánh gió',front:'Đầu xe',wheels:'Bánh xe'};
...
function walk(g,parentGroup='chassis'){
 ...
 let direct=[];
 function flush(){if(direct.length){phase++;for(const p of direct)p.step=phase;steps.push({step:phase,group:currentGroup,parts:direct.map(p=>p.id),name:g.name.replace(/^\d{5} - /,'').replace(/\.ldr$/,'')});direct=[]}}
...
walk(model);
if(parts.length<entry.minimumParts)throw Error('Suspicious part count '+parts.length);
...
const out={partCount:parts.length,phaseCount:phase,geometries:geoMeta,parts,steps,groups,bounds:bounds.getSize(new THREE.Vector3()).multiplyScalar(scale).toArray(),source:entry.source};
await fs.mkdir(output,{recursive:true});await fs.writeFile(path.join(output,'model.json'),JSON.stringify(out));await fs.writeFile(path.join(output,'geometry.bin.gz'),gzipSync(Buffer.concat(buffers),{level:9}));
console.log('DONE',parts.length,phase,geometries.length,offset,groups);

const credits=[];for(const file of [...usedFiles].sort()){const content=await fs.readFile(file,'utf8');credits.push(path.relative(library,file)+'\n'+content.split(/\r?\n/).filter(l=>/^0 (Author:|!LICENSE|Name:)/.test(l)).join('\n'))}await fs.writeFile(path.join(output,'ldraw-part-credits.txt'),credits.join('\n\n'));for(const file of ['CAreadme.txt','CAlicense.txt','CAlicense4.txt'])await fs.copyFile(path.join(library,file),'public/'+file);
